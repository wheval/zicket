...
export function ExplorePageClient({ tickets }: Props) {
  // Capture current time once on mount (pure - only runs once per instance)
  ...
  const filteredTickets = useMemo(() => {
    ...
    if (selectedFilters["Date Range"] !== "All") {
      if (selectedFilters["Date Range"] === "Today") {
        result = result.filter((t) => t.event_date <= mountTime + ONE_DAY);
      } else if (selectedFilters["Date Range"] === "This Week") {
        result = result.filter((t) => t.event_date <= mountTime + ONE_WEEK);
      } else if (selectedFilters["Date Range"] === "This Month") {
        result = result.filter((t) => t.event_date <= mountTime + ONE_MONTH);
      }
    }
    ...
    return result;
  }, [tickets, selectedFilters, sortBy, mountTime]);
